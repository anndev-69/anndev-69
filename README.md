<!--
  ╔══════════════════════════════════════════════════════════════╗
  ║   anndev-69 — GitHub Profile README                          ║
  ║   Theme: Đồng Quê Neon — mộc mạc, hiện đại, có gốc có nghề   ║
  ║   Repo: anndev-69/anndev-69  (public, file: README.md)       ║
  ╚══════════════════════════════════════════════════════════════╝
-->

<!-- ░░░ HERO — Đồng Quê: a full day over the rice terraces (animated SVG) ░░░ -->
<!-- Pinned to the original asset commit so main stays minimal: only README.md + .github/ -->
<div align="center">
  <img width="100%" src="https://raw.githubusercontent.com/anndev-69/anndev-69/b9d7771449f88580eac67c5a8c1f67afc559b664/assets/rice-day-night.svg" alt="Terraced rice paddies through a full day — a farmer leads a water buffalo out at dawn, white egrets glide over the golden fields at noon, and the village house lights up beside the old banyan tree at night" />
</div>

<!-- ░░░ TYPING ░░░ -->
<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1100&color=A855F7&center=true&vCenter=true&width=820&height=70&lines=T%E1%BB%AB+h%E1%BA%A1t+g%E1%BA%A1o+qu%C3%AA+nh%C3%A0+%C4%91%E1%BA%BFn+nh%E1%BB%AFng+d%C3%B2ng+code+%C4%91i+xa;%C4%82n+h%E1%BB%8Dc+n%C3%AAn+ng%C6%B0%E1%BB%9Di+%C2%B7+L%C3%A0m+ngh%E1%BB%81+n%C3%AAn+chuy%E1%BB%87n;Clean+systems+%C2%B7+Humble+roots+%C2%B7+Software+that+ships;L%E1%BB%9Bn+l%C3%AAn+t%E1%BB%AB+%C4%91%E1%BB%93ng+qu%C3%AA+%C2%B7+Tr%C6%B0%E1%BB%9Fng+th%C3%A0nh+c%C3%B9ng+c%C3%B4ng+ngh%E1%BB%87)](https://anguyen.com)

<img src="https://komarev.com/ghpvc/?username=anndev-69&label=PROFILE+VIEWS&color=7C3AED&style=for-the-badge" />
<img src="https://img.shields.io/github/followers/anndev-69?label=FOLLOWERS&style=for-the-badge&color=A855F7&labelColor=0D1117" />
<img src="https://img.shields.io/badge/BASED%20IN-AUSTRALIA-22D3EE?style=for-the-badge&labelColor=0D1117" />

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:7C3AED,50:A855F7,100:22D3EE&height=3&section=header" />

<!-- ░░░ ABOUT ░░░ -->
## <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="28"> `whoami`

```ts
const ann = {
  name:      "ANN-Dev",
  handle:    "@anndev-69",
  role:      "Full-stack Developer",
  roots:     "Vietnamese rice fields",
  focus:     ["clean web systems", "automation tools", "shopify"],
  site:      "anguyen.com",
  motto:     "Go far, stay rooted.",
} as const;
```

**EN** — I grew up on rice from home, carried forward by family, school, and many quiet seasons of hard work. Today I build clean web systems, automation tools, and Shopify solutions that actually ship. The tools are modern; the way I work stays grounded: learn carefully, build honestly, and leave things better than I found them.

**VI** — Mình lớn lên từ hạt gạo quê nhà, được gia đình nuôi dưỡng và chuyện học hành mở lối. Bây giờ mình làm hệ thống web gọn gàng, công cụ tự động hóa và giải pháp Shopify chạy được thật. Công nghệ có thể đi rất xa, nhưng cách mình làm nghề vẫn mộc mạc: học cho tới nơi, làm cho tới chốn, và không quên nơi mình bắt đầu.

<div align="center">

_“Đi xa để mở mang tầm mắt, giữ quê nhà để lòng mình có chốn quay về.”_

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:22D3EE,50:A855F7,100:7C3AED&height=3&section=header" />

<!-- ░░░ TECH STACK ░░░ -->
## <img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="28"> Tech Stack · Công nghệ

> Ngày nhỏ quen nhìn người quê chọn giống, giữ nước, đợi mùa; lớn lên làm phần mềm cũng học được đôi điều: chọn đúng công cụ, chăm từng chi tiết và kiên nhẫn để sản phẩm thành hình.

<div align="center">

`Languages`

<img src="https://skillicons.dev/icons?i=js,ts,python,php,java,cs,cpp,c,go,rust&theme=dark&perline=10" />
<br/>
<img src="https://skillicons.dev/icons?i=ruby,kotlin,swift,dart,lua,perl,r,scala,elixir,bash&theme=dark&perline=10" />

<br/>

`Frontend`

<img src="https://skillicons.dev/icons?i=html,css,react,nextjs,vue,angular,svelte,tailwind,sass,vite&theme=dark&perline=10" />

<br/>

`Backend & Data`

<img src="https://skillicons.dev/icons?i=nodejs,express,nestjs,django,flask,laravel,mysql,postgres,mongodb,redis&theme=dark&perline=10" />

<br/>

`DevOps & Tools`

<img src="https://skillicons.dev/icons?i=git,github,docker,kubernetes,aws,vercel,cloudflare,linux,figma,vscode&theme=dark&perline=10" />

<br/>

`AI Platforms`

<img src="https://img.shields.io/badge/OpenAI-0D1117?style=for-the-badge&logo=openai&logoColor=A855F7" />
<img src="https://img.shields.io/badge/Claude%20Code-0D1117?style=for-the-badge&logo=anthropic&logoColor=22D3EE" />
<img src="https://img.shields.io/badge/xAI%20Grok-0D1117?style=for-the-badge&logo=x&logoColor=F472B6" />
<img src="https://img.shields.io/badge/Gemini-0D1117?style=for-the-badge&logo=googlegemini&logoColor=A855F7" />

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:7C3AED,50:A855F7,100:22D3EE&height=3&section=header" />

<!-- ░░░ STATS ░░░ -->
## <img src="https://media.giphy.com/media/dWesBcTLavkZuG35MI/giphy.gif" width="28"> Stats · Thống kê

_Mỗi ô xanh là một ngày vun xới. Không phải mùa nào cũng rực rỡ, nhưng ngày nào còn học và còn làm thì ngày đó vẫn đang lớn lên._

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=anndev-69&show_icons=true&count_private=true&include_all_commits=true&hide_border=true&bg_color=0D1117&title_color=A855F7&icon_color=22D3EE&text_color=C9D1D9&ring_color=F472B6" />
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=anndev-69&layout=compact&langs_count=8&hide_border=true&bg_color=0D1117&title_color=A855F7&text_color=C9D1D9" />

<br/>

<img src="https://streak-stats.demolab.com?user=anndev-69&hide_border=true&background=0D1117&stroke=7C3AED&ring=A855F7&fire=F472B6&currStreakNum=FFFFFF&sideNums=C9D1D9&currStreakLabel=22D3EE&sideLabels=A855F7&dates=8B949E" />

<br/><br/>

<img width="98%" src="https://github-readme-activity-graph.vercel.app/graph?username=anndev-69&bg_color=0D1117&color=A855F7&line=22D3EE&point=F472B6&area=true&area_color=7C3AED&hide_border=true&custom_title=Contribution%20Pulse" />

<br/>

<img width="98%" src="https://github-profile-trophy.vercel.app/?username=anndev-69&theme=radical&no-frame=true&no-bg=true&column=7&margin-w=6&margin-h=6" />

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:22D3EE,50:A855F7,100:7C3AED&height=3&section=header" />

<!-- ░░░ SNAKE ░░░ -->
## <img src="https://media.giphy.com/media/QssGEmpkyEOhBCb7e1/giphy.gif" width="28"> Contribution Snake

_Con đường học nghề quanh co như bờ ruộng, cứ bền lòng đi rồi những bước nhỏ cũng nối thành một mùa xanh._

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/anndev-69/anndev-69/output/github-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/anndev-69/anndev-69/output/github-snake.svg" />
    <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/anndev-69/anndev-69/output/github-snake-dark.svg" />
  </picture>
</div>


<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:7C3AED,50:A855F7,100:22D3EE&height=3&section=header" />

<!-- ░░░ PROJECTS ░░░ -->
## <img src="https://media.giphy.com/media/LnQjpWaON8nhr21vNW/giphy.gif" width="28"> Featured · Dự án nổi bật

> Đây là những “mùa vụ số” mình đã gieo bằng ý tưởng, chăm bằng code và mang ra dùng ngoài đời — nơi một sản phẩm tốt phải làm được việc trước khi kể được chuyện hay.

<div align="center">

<a href="https://github.com/anndev-69/9router">
  <img width="70%" src="https://github-readme-stats.vercel.app/api/pin/?username=anndev-69&repo=9router&hide_border=true&bg_color=0D1117&title_color=A855F7&icon_color=22D3EE&text_color=C9D1D9" />
</a>

</div>

**Open-source contributions · Đóng góp mã nguồn mở**

- [`decolua/9router`](https://github.com/decolua/9router) — `feat(xai)`: Grok Imagine video generation (`/v1/videos`) + CLI

<!--
  Thêm repo mới sau này: copy khối <a> ở trên, đổi repo=TÊN_REPO.
  Muốn 2 card nằm cạnh nhau thì đổi width="70%" thành width="49%".
-->


<img width="100%" src="https://capsule-render.vercel.app/api?type=rect&color=0:22D3EE,50:A855F7,100:7C3AED&height=3&section=header" />

<!-- ░░░ CONNECT ░░░ -->
## <img src="https://media.giphy.com/media/Ll2fajzk9DgaeriBgg/giphy.gif" width="28"> Connect · Liên hệ

_Nếu bạn cũng thích làm sản phẩm tử tế, nói chuyện công nghệ bên một tách trà, hay đơn giản muốn ghé thăm — mình luôn vui được kết nối._

<div align="center">

<a href="https://anguyen.com"><img src="https://img.shields.io/badge/WEBSITE-anguyen.com-A855F7?style=for-the-badge&logo=googlechrome&logoColor=white&labelColor=0D1117" /></a>
<a href="https://x.com/annguyen"><img src="https://img.shields.io/badge/X-@annguyen-22D3EE?style=for-the-badge&logo=x&logoColor=white&labelColor=0D1117" /></a>
<a href="https://github.com/anndev-69"><img src="https://img.shields.io/badge/GITHUB-anndev--69-F472B6?style=for-the-badge&logo=github&logoColor=white&labelColor=0D1117" /></a>

</div>

<br/>

<div align="center">
  <img src="https://github-readme-quotes-bay.vercel.app/quote?theme=radical&animation=grow_out_in&layout=default&font=Fira%20Code" />
</div>

<!-- ░░░ FOOTER WAVE ░░░ -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:22D3EE,35:A855F7,70:7C3AED,100:0D1117&height=140&section=footer&text=from%20the%20rice%20fields%2C%20with%20care&fontSize=22&fontColor=FFFFFF&fontAlignY=72&animation=twinkling" />
